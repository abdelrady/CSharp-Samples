...
namespace MergeFilesIntoTable
{
    class Program
    {
        ...
        {
            //ImportDiffFilesToTable(new[]{@"C:\Users\abdel\Downloads\staar_to_upload\final", "*.csv", @".\sql2016", "g2",
            //    "merge"});
            ImportFilesToTables(new[]{ @"D:\Temp\latlong", "*.csv", @".\sql2016", "staar"});
        }

        private static void ImportFilesToTables(string[] args)
        {
        ...
        private static void CreateTableFromHeader(string server, string database, string tableName, string fileHeader)
        {
            ...
            var createTableScript = "CREATE TABLE " + tableName + "(" + fileHeader + " VARCHAR(1024))";
            new SqlCommand(dropTableScript, cnn).ExecuteNonQuery();
            new SqlCommand(createTableScript, cnn).ExecuteNonQuery();
            cnn.Close();
            cnn.Dispose();
